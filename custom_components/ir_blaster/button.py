...
from .ir_packet import TRIGGER_PACKET, build_code_packet
...
async def _send_ir(hass, topic: str, hex_code: str) -> None:
    """Send an IR code as two separate SerialSend5 packets."""
    code_pkt = build_code_packet(hex_code)
    if not code_pkt:
        _LOGGER.error("Invalid hex code: %s", hex_code)
        return
    send_topic = TOPIC_SEND.format(topic=topic)
    await mqtt.async_publish(hass, send_topic, TRIGGER_PACKET)
    await mqtt.async_publish(hass, send_topic, code_pkt)
    _LOGGER.debug("IR sent — trigger: %s  code: %s", TRIGGER_PACKET, code_pkt)

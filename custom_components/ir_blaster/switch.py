"""Switch platform for IR Blaster — study mode."""
...
from .const import CONF_DEVICE_NAME, CONF_TOPIC, DOMAIN, PKT_STUDY_OFF, PKT_STUDY_ON, TOPIC_SEND
...
class IRStudyModeSwitch(SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs):
        await mqtt.async_publish(
            self._hass, TOPIC_SEND.format(topic=self._topic), PKT_STUDY_ON
        )
    ...
    async def async_turn_off(self, **kwargs):
        await mqtt.async_publish(
            self._hass, TOPIC_SEND.format(topic=self._topic), PKT_STUDY_OFF
        )
        self._attr_is_on = False
        self.async_write_ha_state()
